...
namespace DISSample {
	...
	public class Sample : System.Web.Services.WebService 	{ 
		...
		public string Login(){ 
			...
			sCmd += "<CompanyUsername>" + "PRO0003" + "</CompanyUsername>"; 
		...
		public System.Xml.XmlDocument GetEmptyBPXml( string SessionID ) 		{ 
           ...
           // strXML = @"<?xml version=""1.0"" encoding=""UTF-16""?>" + @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + "<env:Header>" + "<SessionID>" + SessionID + "</SessionID>" + @"</env:Header><env:Body><dis:GetItemList xmlns:dis=""http://www.sap.com/SBO/DIS"">" + "</dis:GetItemList>"+ "</env:Body>"+ "</env:Envelope>";
            //strXML = @"<?xml version=""1.0"" encoding=""UTF-16""?>" + @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + "<env:Header>" + "<SessionID>" + SessionID + "</SessionID>" + @"</env:Header><env:Body><dis:GetBusinessObjectTemplate xmlns:dis=""http://www.sap.com/SBO/DIS"">" + "<CardType>cCustomer</CardType>" + "</dis:GetBusinessObjectTemplate>" + "</env:Body>" + "</env:Envelope>";
			s = n.Interact( strXML ); 
			d.LoadXml( s ); 
			return ( RemoveEnv( d ) ); 
		}
       ...
       // [ WebMethod() ]
        public String addBP(String SessionID,String cardCode, String nombre,String telefono,String email_cli,String nombreRazon, String RFC,String calle, String colonia,String ciudad,String municipio,String estado,String cp,String numero, String en_calle,String en_colonia, String en_ciudad,String en_municipio,String en_estado,String en_cp,String en_numero){
            ...
            sCmd = @"<?xml version=""1.0"" encoding=""UTF-16""?>";
            sCmd += @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">";
            sCmd += "<env:Header><SessionID>" + SessionID + "</SessionID></env:Header><env:Body>";// ID de Sesion
            sCmd += @"<dis:AddObject xmlns:dis=""http://www.sap.com/SBO/DIS"" CommandID=""Add BP"">";
            sCmd += "<BOM><BO><AdmInfo><Object>oBusinessPartners</Object></AdmInfo>";
            sCmd += "<BusinessPartners>";
            sCmd += "<row>";//datos de facturacion
            sCmd += "<CardCode>" + cardCode + "</CardCode>";//identificador de Customer (C00XXX)
            sCmd += "<CardName>" + nombreRazon + "</CardName>"; //nombre de la empresa
            ...
            sCmd += "<ContactPerson>" + nombre + "</ContactPerson>";//nombre de contacto
            ...
            sCmd += "<row>";
            sCmd += "<CardCode>"+cardCode+"</CardCode>";
            sCmd += "<Name>"+nombreRazon+"</Name>";
            sCmd += "<Position>manager</Position>";
            sCmd += "<Phone1>"+telefono+"</Phone1>";
            sCmd += "<E_Mail>"+email_cli+"</E_Mail>";
            sCmd += "<InternalCode>613</InternalCode>";
            ...
            sCmd += "<BPAddresses>";
            sCmd +=    "<row>";
            sCmd += "<AddressName>Entrega</AddressName>";
            sCmd += "<Street>"+calle+"</Street>";
           // sCmd += "<State>"+estado+"</State>";
            sCmd += "<ZipCode>"+cp+"</ZipCode>";
            sCmd += "<City>"+ciudad+"</City>";
            sCmd += "</row>";
            sCmd += "</BPAddresses>";//direccion de contacto
            sCmd += "</BO></BOM></dis:AddObject></env:Body></env:Envelope>";
        ...
        public String getByKey(string SessionID) { 
            ...
            d = new System.Xml.XmlDocument();
            n = new SBODI_Server.Node();
            strXML = @"<?xml version=""1.0"" encoding=""UTF-16""?>" + @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + "<env:Header>" + "<SessionID>" + SessionID + "</SessionID>" + @"</env:Header> <env:Body> <dis:GetByKey xmlns:dis=""http://www.sap.com/SBO/DIS"">" + "<CardType>cCustomer</CardType>" + "<CardCode>C00272</CardCode>" + "</dis:GetByKey> </env:Body></env:Envelope>";
            s = n.Interact(strXML);
            d.LoadXml(s);
            return AsString(RemoveEnv(d));
        ...
        public String detalleProducto(string SessionID)
        {
            ...
            strXML = @"<?xml version=""1.0"" encoding=""UTF-16""?>" + @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + "<env:Header>" + "<SessionID>" + SessionID + "</SessionID>" + @"</env:Header><env:Body><dis:GetBusinessObjectTemplate xmlns:dis=""http://www.sap.com/SBO/DIS"">" + "<Object>oCustomer</Object>" + "</dis:GetBusinessObjectTemplate></env:Body></env:Envelope>";
            //strXML = @"<?xml version=""1.0"" encoding=""UTF-16""?>" + @"<env:Envelope xmlns:env=""http://schemas.xmlsoap.org/soap/envelope/"">" + "<env:Header>" + "<SessionID>" + SessionID + "</SessionID>" + @"</env:Header><env:Body><dis:GetItemList xmlns:dis=""http://www.sap.com/SBO/DIS"">" + "</dis:GetItemList>" + "</env:Body>" + "</env:Envelope>";
            s = n.Interact(strXML);
            d.LoadXml(s);
            return AsString(RemoveEnv(d));
        ...
        public String SumCustomer(String customer) {
            String numero;
            int final;
            numero = customer.Trim(new Char[] { 'C' });
            final = Convert.ToInt32(numero);
            final++;
            return "C00" + final;
        }
        [WebMethod()]
         ...
         */ 
        [WebMethod()]
        public String getfinalCustomer(String id) {
            SBODI_Server.Node DISnode = null;
            string sSOAPans = null, sCmd = null;
            DISnode = new SBODI_Server.Node();
            sCmd = @"<?xml version=""1.0"" encoding=""UTF-16""?>";
            ...
            sCmd += "<env:Body>";
            sCmd += @"<dis:ExecuteSQL xmlns:dis=""http://www.sap.com/SBO/DIS"">";
            sCmd += "<DoQuery>select top 1 CardCode from OCRD where CardCode LIKE 'C%' ORDER BY CardCode DESC</DoQuery>";
            sCmd += "</dis:ExecuteSQL>";
            sCmd += "</env:Body>";
            sCmd += "</env:Envelope>";
